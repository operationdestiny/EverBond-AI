...
export function CharacterGalleryClient({
  ...
  async function loadGallery() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error || copy.mediaError);
      }
    ...
    try {
      ...
      setData((current) =>
        current
          ? {
              ...current,
              images: [payload.image, ...current.images]
            }
          : current
      );
      setPrompt("");
            ...
            <section className="overflow-hidden rounded-[2.25rem] border border-bond-rose/45 bg-[linear-gradient(135deg,rgba(255,92,168,0.12),rgba(255,255,255,0.025),rgba(89,45,130,0.14))] p-7 shadow-[0_0_48px_rgba(255,92,168,0.10)] md:p-10">
              <div className="grid gap-8 lg:grid-cols-[280px_1fr] lg:items-center">
                <img
                  ...
                  className="aspect-[4/5] w-full rounded-[1.75rem] border border-bond-rose/35 object-cover"
                />

                <div>
                  <p className="text-sm font-bold uppercase tracking-[0.24em] text-bond-rose">
                    PRIVATE GALLERY
                  </p>
                  ...
                  <p className="mt-3 text-sm font-semibold text-bond-rose">
                  ...
                  <textarea
                    ...
                    rows={4}
                    maxLength={1500}
                    className="mt-6 w-full resize-none rounded-[1.5rem] border border-white/10 bg-black/25 px-5 py-4 text-white outline-none placeholder:text-bond-muted focus:border-bond-rose/65"
                  />
                      ...
                      >
                        ...
                        <div className="p-4">
                          ...
                          <button
                            ...
                            disabled={selected}
                            className={`mt-4 flex w-full items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-bold ${
                              selected
                          ...
                          <button
                            ...
                            className="mt-2 flex w-full items-center justify-center gap-2 rounded-full border border-red-400/25 bg-red-500/10 px-4 py-2.5 text-sm font-bold text-red-100"
                          >
                            <Trash2 size={16} />
                            {copy.deleteImage}
                          </button>
