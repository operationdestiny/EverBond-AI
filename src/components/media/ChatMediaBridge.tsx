...
export function ChatMediaBridge() {
  ...
  useEffect(() => {
    ...
    function attach() {
      ...
      if (!mount) {
        ...
        mount.className = "mx-auto mb-2 flex max-w-4xl flex-wrap items-center gap-2";
    ...
    void fetch(
      ...
      .then(async (response) => {
        ...
        if (!cancelled && response.ok && payload?.character) {
          ...
          setDisplayImage(payload.character.image);
        }
      })
      .catch(() => undefined);
      ...
      .then(async (response) => {
        ...
        if (!cancelled && response.ok) {
          const selected = Array.isArray(payload.images)
            ? payload.images.find(
                (image: { id: string }) =>
                  image.id === payload.selectedImageId
              )
            : null;
  ...
  useEffect(() => {
    function refreshSelectedImage(
      ...
      if (session?.access_token && character) {
        void fetch(
          `/api/character-gallery/${encodeURIComponent(slug)}`,
          {
            headers: {
  ...
  const toolbar = (
    <>
      <button
        type="button"
        onClick={() =>
          requireSession(() => setCallOpen(true))
        }
        className="bond-pink-button inline-flex items-center gap-2 rounded-full bg-bond-rose px-5 py-2.5 text-sm font-bold text-white shadow-[0_0_20px_rgba(255,92,168,0.18)]"
      >
        <Phone size={16} />
      ...
      <Link
        href={
          session
            ? `/character/${character.slug}/gallery`
            : "#"
        }
        onClick={(event) => {
          if (!session) {
        ...
        className="bond-pink-button inline-flex items-center gap-2 rounded-full border border-bond-rose/60 bg-bond-rose/10 px-5 py-2.5 text-sm font-bold text-white"
      >
        <ImageIcon size={16} />
