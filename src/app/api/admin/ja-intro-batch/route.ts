...
import sourceRows from "@/data/chat-intro-translations/ja-source.json";
...
function validateTranslated(
  ...
  return source.map((row) => {
    ...
    if (!hasJapanese(opening) || !hasJapanese(first)) {
      throw new Error(`NON_JAPANESE_TRANSLATION:${row.id}`);
    }
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a strict English-to-Japanese translation engine for EverBond. " +
            ...
            "Preserve character names, EverBond, EverCoin, Ever Memory™, URLs, IDs, asterisks, quotation marks, ellipses, line breaks, and punctuation as closely as Japanese grammar permits. " +
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("JA_INTRO_BATCH_FAILED", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "JA_INTRO_BATCH_FAILED"
      },
      { status: 500 }
    );
  }
