...
import sourceRows from "@/data/chat-intro-translations/fr-source.json";
...
const JOB_TOKEN_SHA256 = "5b537057b2a9fa0fc074a75d64ffddbafb3f7910ac2075f0f3b2aaea3e956150";
const LANGUAGE = "FR";
const TRANSLATOR = "venice:one-time-fr-intro-v1";
const BATCH_SIZE = 4;
...
function looksFrench(value: string) {
  ...
  return (
    ...
    /\b(le|la|les|un|une|des|de|du|je|tu|il|elle|nous|vous|ils|elles|et|est|sont|que|qui|dans|sur|avec|pour|pas|plus|comme|ce|ça|mon|ma|mes|ton|ta|tes|son|sa|ses|au|aux|en|mais|si|quand|alors|ici|là)\b/i.test(normalized)
  );
}
...
function validateTranslated(
  ...
  return source.map((row) => {
    ...
    if (!looksFrench(`${opening} ${first}`)) {
      throw new Error(`NON_FRENCH_TRANSLATION:${row.id}`);
    }
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a strict English-to-French translation engine for EverBond. " +
            ...
            "Preserve character names, EverBond, EverCoin, Ever Memory™, URLs, IDs, asterisks, quotation marks, ellipses, line breaks, and punctuation as closely as French grammar permits. " +
...
async function translateWithFallback(source: SourceRow[]): Promise<TranslationRow[]> {
...
async function runBatch(limit: number) {
  ...
  for (let index = 0; index < batches.length; index += CONCURRENCY) {
    ...
    for (let groupIndex = 0; groupIndex < group.length; groupIndex += 1) {
      ...
      const rows = sourceBatch.map((row) => {
        const item = byId.get(row.id);
        if (!item) throw new Error(`MISSING_TRANSLATION_AFTER_VALIDATE:${row.id}`);
        return {
          character_id: row.id,
          language: LANGUAGE,
      ...
      const { error } = await supabase
        .from("character_chat_translations")
        .upsert(rows, { onConflict: "character_id,language" });
...
async function exportPage(offset: number, limit: number) {
  ...
  return {
    ...
    nextOffset: offset + slice.length < SOURCE.length
      ? offset + slice.length
      : null,
    items
  };
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (action === "export") {
      ...
      const offset = Math.min(Math.max(Math.trunc(rawOffset), 0), SOURCE.length);
    ...
    console.error("FR_INTRO_BATCH_FAILED", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "FR_INTRO_BATCH_FAILED"
      },
      { status: 500 }
    );
  }
