...
import sourceRows from "@/data/chat-intro-translations/ko-source.json";
...
const JOB_TOKEN_SHA256 = "60e1edeba6ea10bf68e3129dbe92cb9fd471a1f4cfb760eb1d157ca12282629f";
const LANGUAGE = "KO";
const TRANSLATOR = "venice:one-time-ko-intro-v1";
const BATCH_SIZE = 4;
...
function extractJson(text: unknown) {
  ...
  if (start < 0 || end <= start) {
    throw new Error("INVALID_VENICE_JSON");
  }
...
function validateTranslated(
  ...
  if (!Array.isArray(items)) {
    throw new Error("MISSING_TRANSLATION_ITEMS");
  }
  ...
  return source.map((row) => {
    ...
    if (!translated) {
      throw new Error(`MISSING_TRANSLATION:${row.id}`);
    }
    ...
    if (!opening || !first) {
      throw new Error(`BLANK_TRANSLATION:${row.id}`);
    }

    if (
      opening === row.openingScenario ||
      first === row.firstMessage
    ) {
      throw new Error(`UNCHANGED_TRANSLATION:${row.id}`);
    }

    if (!looksKorean(`${opening} ${first}`)) {
      throw new Error(`NON_KOREAN_TRANSLATION:${row.id}`);
    }
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a strict English-to-Korean translation engine for EverBond. " +
            ...
            "Preserve character names, EverBond, EverCoin, Ever Memory™, URLs, IDs, asterisks, quotation marks, ellipses, line breaks, and punctuation as closely as Korean grammar permits. " +
  ...
  for (let index = 0; index < batches.length; index += CONCURRENCY) {
    ...
    for (let groupIndex = 0; groupIndex < group.length; groupIndex += 1) {
      ...
      const dbRows = sourceBatch.map((row) => {
        ...
        if (!item) {
          throw new Error(
            `MISSING_TRANSLATION_AFTER_VALIDATE:${row.id}`
          );
        }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("KO_INTRO_BATCH_FAILED", error);
